...
func (fc *FanController) tick() {
	...
	if err != nil {
		...
		// In case of error, maybe we should not change the fan speed
		// or set a failsafe speed. For now, we'll just log and skip the tick.
		return
	}
	fc.lastTemp = temp

	// Panic Mode: if temp exceeds critical, hand over control to BIOS and stop.
	if temp > float64(fc.config.CriticalTemperature) {
		log.Printf("CRITICAL: Temperature %.1f°C exceeds threshold of %d°C. Releasing control to BIOS.", temp, fc.config.CriticalTemperature)
		fc.releaseAllFansToBios()
	...
	for i, fanConfig := range fc.config.FanConfigurations {
		...
		switch state.Mode {
		case ModeAuto:
			targetSpeedPercent = calculateSpeedForTemp(temp, fanConfig.TemperatureThresholds)
