...
func NewEcDriver() (*EcDriver, error) {
	dll := syscall.NewLazyDLL("inpoutx64.dll")
	if err := dll.Load(); err != nil {
		return nil, fmt.Errorf("critical: failed to load inpoutx64.dll: %w. Ensure the DLL is in the application directory", err)
	}
	...
	if out32.Find() != nil || inp32.Find() != nil {
		return nil, fmt.Errorf("critical: could not find Inp32/Out32 procedures in inpoutx64.dll")
	}
