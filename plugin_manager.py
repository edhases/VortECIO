...
class PluginManager:
    ...
    def discover_plugins(self):
        ...
        if not os.path.exists(self.plugin_folder):
            logger.warning(f"Plugin folder not found: {self.plugin_folder}")
            return
    ...
    def load_plugin_package(self, path):
        ...
        if not os.path.exists(init_file):
            logger.warning(f"Plugin __init__.py not found: {path}")
            return
        ...
        try:
            ...
            if hasattr(module, 'register'):
                logger.info(f"Registering plugin: {package_name}")
                plugin_instance = module.register(self.app_logic)

                if plugin_instance:
                    self.plugins.append(plugin_instance)
                    logger.info(f"Plugin {package_name} loaded successfully")
                else:
                    logger.error(f"Plugin {package_name} registration failed")
            else:
                logger.error(f"Plugin {package_name} has no register() function")

        except Exception as e:
            logger.error(f"Failed to load plugin {package_name}: {e}", exc_info=True)
